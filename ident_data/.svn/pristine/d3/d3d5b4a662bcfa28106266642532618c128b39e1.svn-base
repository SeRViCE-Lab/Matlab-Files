...
% This example shows linear model identification of a glass tube
...
%%
% The sample time of the data is 1 second, while the process time
...
grid on
%%
% Note that the input has very little relative energy above 1 rad/sec while
% the output contains relatively larger values above that frequency. There
% are thus some high frequency disturbances that may cause some problem for
% the model building.
...
Imp = impulseest(ze,[],'negative',impulseestOptions('RegulKernel','SE'));
...
%%
% We again see that the delay is about 3 samples (which is consistent with
% what we saw above; 12 second delay with sample time of 4 seconds in
% zde). Let us now try estimating a default model, where the order is
% automatically picked by the estimator.

Mdefault = n4sid(zde);
compare(zdv,Mdefault)
%%
% The estimator picked a 4th order model. It seems to provide a better fit
...
%%
% The model |m2| is about same as |Mdefault| is fitting the data but uses
% lower orders.
...
%%
% From the pole-zero plot, there is am indication of pole-zero
...
% well with lower order models. Try a [1 1 3] ARX model: 
m3 = arx(zde,[1 1 3])
...
%%
% The three models deliver comparable results. Similarly, we can compare
% the 5-step ahead prediction capability of the models:
compare(zdv,Mdefault,m2,m3,5)

%%
% As these plots indicate, a reduction in model order does not
% significantly reduce its effectiveness is predicting future values.
