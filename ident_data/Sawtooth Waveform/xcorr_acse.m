function [c,lags] = xcorr_acse(a, b, maxlag, option)
%
%XCORR Cross-correlation function estimates.
%   C = XCORR_ACSE(A,B), where A and B are length M vectors (M>1), returns
...
%   column vector.
%
%   XCORR_ACSE(A), when A is a vector, is the auto-correlation sequence.   
%   XCORR_ACSE(A), when A is an M-by-N matrix, is a large matrix with
...
%
%   XCORR_ACSE(A,B,MAXLAG) computes the cross-correlation over the
%   range of lags:  -MAXLAG to MAXLAG, i.e., 2*MAXLAG+1 lags.
%   XCORR_ACSE(A,MAXLAG) computes the auto-correlation over the
%   range of lags.   If missing, default is MAXLAG = M-1.
%
%   [C,LAGS] = XCORR_ACSE  returns a vector of lag indices (LAGS).
%
%   XCORR_ACSE(A,'flag'), XCORR_ACSE(A,B,'flag') or XCORR_ACSE(A,B,MAXLAG,'flag') 
...
%
%   See also XCORR,XCOV, CORRCOEF, CONV, COV and XCORR2.
...
option = lower(option);
%
%   Add the following three statements by myself
%
nab=length(a);
vara=nab*var(a);
varb=nab*var(b);
