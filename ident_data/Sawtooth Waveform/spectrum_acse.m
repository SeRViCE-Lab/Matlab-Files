function [Spec,f] = spectrum_acse(varargin)
...
%      P = [Pxx Pyy Pxy Pxy Cxy Pxxc Pyyc Pxyc]
%   where
...
ff=round(length(freq_vector)/2-1);
fv=freq_vector(1:ff);
if nargout == 0,   % do plots
   newplot;
   fprintf('   5.1  Power spectrum of detrended iput\n\n');
    ...
    %	freq_vector,c(:,2),'--');
    %
    semilogy(fv,Pxx(1:ff),fv,c(1:ff,1),'--',fv,c(1:ff,2),'--');
	title('S11(f)- Power Spectral Density of Detrended Input')
	...
	pause
        ...
        newplot;
    fprintf('   5.2  Power spectrum of detrended output\n\n');
    ...
    %      		 freq_vector,c(:,2),'--');
    %
	semilogy(fv,Pyy(1:ff),fv,c(1:ff,1),'--',fv,c(1:ff,2),'--');
	title('S22(f)-Power Spectral Density of Detrended Output')
	xlabel('Frequency [Hz]'),grid,
    fprintf('        [Press ANY to continue...]\n\n');
	pause

        newplot;
    fprintf('   5.3  Gain spectrum from input to output\n\n');
	subplot(111),
    %
    %   This is for 0-1Hz
    %
    ...
    %
	semilogy(fv,abs(Txy(1:ff)));
	title('G21(f)- Gain Spectrum'),
	xlabel('Frequency [Hz]'),grid,
    fprintf('        [Press ANY to continue...]\n\n');
	pause

        newplot;
    fprintf('   5.4  Phase spectrum from input to output\n\n');
	subplot(111),
    %
    %   This is for 0-1Hz
    %
    ...
    %
	plot(fv,180/pi*angle(Txy(1:ff))),
    title('PHY(f)- Phase Spectrum'),
	xlabel('Frequency [Hz]'),grid,
    fprintf('        [Press ANY to continue...]\n\n');
	pause

        newplot;
    fprintf('   5.5  Squared coherency between input and output\n\n');
	subplot(111),
    %
    %   This is for 0-1Hz
    %
